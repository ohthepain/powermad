...
import {ProbabilityFilter} from './probability-filter';
...
const ProbabilityLane = (props: ProbabilityLaneProps) => {
    ...
    const filter: ProbabilityFilter = new ProbabilityFilter()

    useEffect(() => {
        console.log(`ProbabilityLane: program changed to ${JSON.stringify(program)}`)
    }, [program.steps])
    ...
    const handleDragBars = (offset: number, index: number) => {
        ...
        console.log(`ProbabilityLane.handleValueChange: index ${index} value ${newValue} ==> ${JSON.stringify(newsteps)}`)
...
export default ProbabilityLane
