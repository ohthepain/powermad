...
//
function buildMidiNumberAttributes(midiNumber: number) : any {
...
function buildMidiNumberAttributesCache() : Array<any> {
    return range(MIN_MIDI_NUMBER, MAX_MIDI_NUMBER + 1).reduce((cache: Array<any>, midiNumber: number) => {
        cache[midiNumber] = buildMidiNumberAttributes(midiNumber);
        return cache;
    }, []);
}

const midiNumberAttributesCache : Array<any> = buildMidiNumberAttributesCache();

// Returns an object containing various attributes for a given MIDI number.
// Throws error for invalid midiNumbers.
function getAttributes(midiNumber: number): any {
    const attrs : any = midiNumberAttributesCache[midiNumber];
    if (!attrs) {
